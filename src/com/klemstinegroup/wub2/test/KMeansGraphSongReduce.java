...
public class KMeansGraphSongReduce {
    ...
    public static JFrame frame = new JFrame(KMeansGraphSongReduce.class.toString());
...
//                System.out.println("dist="+dd);
        ...
        Digraph graph=new Digraph(3000);
        int startNode=0;
...
//            AudioInterval ai = tempSong.getAudioInterval(tempSong.analysis.getSegments().get(tem.indexOf(play.segment)));
            graph.addEdge(startNode,play.segment);
            startNode=play.segment;
//            ai.payload = play;
//            audio.play(ai);
        }
        graph.addEdge(startNode,numClusters);
        ...
        while(startNode!=numClusters){
            ...
            Iterator<Integer> adj = graph.adj(startNode).iterator();
